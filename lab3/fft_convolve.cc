...
#define STOP_RECORD_TIMER(name) {                           \
    ...
    for (int ch = 0; ch < nChannels; ch++){
        ...
        Set the rest of the memory regions to 0 using cudaMemset, but setting
        entirety of array to 0 and them Memcpying relevant portions.
        */
        ...
        Note that input_data only stores x[n] as read from the input audio file so only
        copy N cufftComplex over. */
        ...
        /* Create a cuFFT plan for the forward and inverse transforms.
           (We use the same plan for both, using CUFFT_FORWARD/CUFFT_INVERSE) */

        cufftHandle plan; // cuFFT uses plans similar to FFTW for config info
        int batch = 1; // Number of transforms of given size for this plan; see API ref 3.2.1
        ...
        /* Run the inverse DFT on the output signal, done in-place. */
        ...
        // Divide kernel pointwise by the max, for scaling.
        ...
        if ( fabs(max_abs_val_fromGPU - max_abs_val) > 1e-4 ){
...
int main(int argc, char **argv){
    ...
    int max_time_allowed_in_seconds = 120;
