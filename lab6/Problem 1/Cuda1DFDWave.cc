...
int main(int argc, char* argv[]) {
...

  {
    ...
    /* Create GPU memory for calculations.
    As an initial condition at time 0, zero out your memory as well. */
    size_t data_len = (numberOfNodes * 3) * sizeof(float);
    // This will function as the pointer array 'data' above, but in device memory
    float *dev_data;
    cudaMalloc( (void **) &dev_data, data_len);
    cudaMemset( dev_data, 0, data_len);
    ...
    for (size_t timestepIndex = 0; timestepIndex < numberOfTimesteps;
        ...
        float *dev_oldDisplacements = dev_data + ((timestepIndex - 1) % 3) * numberOfNodes;
        float *dev_curDisplacements = dev_data + ((timestepIndex    ) % 3) * numberOfNodes;
        float *dev_newDisplacements = dev_data + ((timestepIndex + 1) % 3) * numberOfNodes;

        // Perform the time-step GPU-side
        cudaCall1DWaveEqnKernel(blocks, threadsPerBlock, numberOfNodes, courant,
            dev_oldDisplacements, dev_curDisplacements, dev_newDisplacements,
            left_boundary_value);
        ...
        if (CUDATEST_WRITE_ENABLED == true && numberOfOutputFiles > 0 &&
            ...
            cudaMemcpy(file_output, dev_curDisplacements, numberOfNodes * sizeof(float),
