...
#include "host/host_stencil_chebyshev1d.hpp"
#include "gpu/gpu_stencil_chebyshev1d.hpp"
...
template <typename ValueType>
LocalStencil<ValueType>::LocalStencil(unsigned int type, bool accel) {

  LOG_DEBUG(this, "LocalStencil::LocalStencil()",
            "constructor with stencil type");

  assert( (type == Laplace2D) || (type == Chebyshev1D) ); // the only two at the moment
  ...
  } else if (type == Chebyshev1D) {
    if (accel) {
      this->stencil_accel_ = new GPUAcceleratorStencilChebyshev1D<ValueType>(this->local_backend_);
      this->stencil_ = this->stencil_accel_;
    } else {
      this->stencil_host_ = new HostStencilChebyshev1D<ValueType>(this->local_backend_);
      this->stencil_ = this->stencil_host_;
    }
  }
...
template <typename ValueType>
bool LocalStencil<ValueType>::is_host(void) const {
  return (this->stencil_ == this->stencil_host_);
}

template <typename ValueType>
bool LocalStencil<ValueType>::is_accel(void) const {
  return (this->stencil_ == this->stencil_accel_);
}

template <typename ValueType>
void LocalStencil<ValueType>::Plan(ValueType *p, ValueType *q, ValueType *x_a, ValueType *x_b) {
    LOG_DEBUG(this, "LocalStencil::Plan()",
              "base Plan() called");

    //assert( type == Chebyshev1D );
    if (is_host()) {
      this->stencil_host_->Plan(p, q, x_a, x_b);
    } else {
      this->stencil_accel_->Plan(p, q, x_a, x_b);
    }
}
